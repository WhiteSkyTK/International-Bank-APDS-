        ...
        <DashboardLayout title="Dashboard Overview">
            <div className="space-y-6">
                <div className="bg-[#1C4382] rounded-[2rem] p-8 lg:p-10 text-white shadow-2xl relative overflow-hidden" style={{ minHeight: '15rem' }}>
                    ...
                    <p className="text-blue-300 text-sm mb-6">≈ {ccyInfo.flag} {ccyInfo.symbol}{balanceFx} {selectedCcy}</p>
                    <div className="flex flex-wrap gap-x-10 gap-y-2 text-sm z-10">
                        <div><p className="text-[11px] text-blue-200 mb-0.5">Account</p><p className="font-bold">···· {user.accountNumber?.slice(-4) ?? '????'}</p></div>
                        <div><p className="text-[11px] text-blue-200 mb-0.5">Type</p><p className="font-bold">Cheque Account</p></div>
                        <div><p className="text-[11px] text-blue-200 mb-0.5">Branch</p><p className="font-bold">250655</p></div>
                    </div>
                    <div className="absolute right-6 bottom-6 flex gap-3">
                        <button onClick={() => navigate('/payment')} className="bg-white text-[#1C4382] px-5 py-2.5 rounded-full font-bold shadow-lg hover:scale-105 transition text-sm">+ Make Payment</button>
                        <button onClick={downloadStatement} className="bg-white/10 border border-white/20 px-5 py-2.5 rounded-full font-bold hover:bg-white/20 transition flex items-center gap-2 text-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center">
                        <p className="text-gray-400 text-[11px] font-medium mb-1 uppercase">Sent Recently</p>
                        <p className="text-lg font-bold text-gray-800">R {totalSent.toLocaleString('en-ZA', { minimumFractionDigits: 2 })}</p>
                    </div>
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center">
                    ...
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center">
                        ...
                        <p className="text-lg font-bold text-gray-800">{recentTx.filter((t) => t.status === 'Pending').length}</p>
                    </div>
                    <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
                        ...
                        <select value={selectedCcy} onChange={(e) => setSelectedCcy(e.target.value)}
                        ...
                        <p className="text-[10px] text-gray-400">1 ZAR = {rate.toFixed(4)} {selectedCcy}</p>
                        <p className="text-sm font-bold text-[#1C4382] mt-1">{ccyInfo.symbol}{balanceFx}</p>
                    </div>
                </div>
                ...
                <div>
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button onClick={() => navigate('/transactions')} className="text-xs font-bold text-[#4A80D4] hover:underline">View All →</button>
                    </div>
                    {txLoading && <div className="flex items-center justify-center py-10 gap-3 text-gray-400"><Loader2 size={20} className="animate-spin" /><span className="text-sm">Loading…</span></div>}
                    {!txLoading && txError && <p className="text-red-500 text-xs font-semibold p-4 bg-red-50 rounded-2xl border border-red-100">{txError}</p>}
                    {!txLoading && !txError && recentTx.length === 0 && (
                        <div className="bg-white rounded-3xl p-8 text-center text-gray-400 border border-gray-100">
                            <p className="text-2xl mb-2">📭</p><p className="text-sm font-medium">No transactions yet.</p>
                        </div>
                    )}
                    <div className="space-y-3">
                        {!txLoading && recentTx.map((tx) => (
                            <div key={tx._id} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 flex items-center justify-between hover:shadow-md transition">
                                <div className="flex items-center gap-4">
                                    <div className="w-11 h-11 bg-red-50 text-red-400 rounded-2xl flex items-center justify-center shrink-0"><ArrowUpRight size={18} /></div>
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-400 mt-0.5">{new Date(tx.createdAt).toLocaleString('en-ZA', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="font-bold text-red-500 text-sm">-{tx.currency ?? 'R'} {tx.amount.toLocaleString('en-ZA', { minimumFractionDigits: 2 })}</p>
